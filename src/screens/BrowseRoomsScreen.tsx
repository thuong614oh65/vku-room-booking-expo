...
export const BrowseRoomsScreen: React.FC = () => {
  ...
  // L?c danh sách phòng theo search & chips da tham s?
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      ...
      // 4. Equipment filter (ph?i th?a mãn t?t c? trang thi?t b? du?c ch?n)
  ...
  // Ki?m tra phòng có slot tr?ng hôm nay không
  ...
  const isRoomAvailableToday = useCallback(
    (roomId: string) => {
      ...
      return todayBookingsCount < 5; // Có 5 slot/ngày, n?u < 5 là còn tr?ng
    },
    [bookings, todayStr]
  );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.schoolHeader}>VKU CAMPUS • H? TH?NG Ð?T PHÒNG H?C</Text>
          <Text style={styles.screenTitle}>Tra C?u Phòng H?c & Lab</Text>
        </View>
      ...
      <View style={styles.resultBanner}>
        <Text style={styles.resultCountText}>
          Tìm th?y <Text style={styles.resultCountBold}>{filteredRooms.length}</Text> phòng h?c & lab kh? d?ng
        </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyTitle}>Không tìm th?y phòng phù h?p</Text>
            <Text style={styles.emptySub}>
              Vui lòng th? n?i l?ng b? l?c s?c ch?a ho?c trang thi?t b? d? xem thêm k?t qu?.
            </Text>
          </View>
        }
      />
